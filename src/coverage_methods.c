		...
		int cvg_len)
{
	...
	if (SEids_len == 0) {
		...
		zero = 0;
		return integer_Rle_constructor(&zero, 1, &cvg_len, 0);
	}
	...
	return integer_Rle_constructor(values_buf, buf_len, lengths_buf, 0);
}
		...
		int cvg_len)
{
	...
	if (SEids_len == 0) {
		//return an Rle with one run of 0's
		zero = 0.0;
		return numeric_Rle_constructor(&zero, 1, &cvg_len, 0);
	}
	sort_SEids(SEids, SEids_len, x_start, x_width);
	buf_len = SEids_len + 1;
	...
	compute_double_coverage_in_bufs(SEids, SEids_len,
			x_start, x_width, weight, weight_len, cvg_len,
			values_buf, lengths_buf);
	return numeric_Rle_constructor(values_buf, buf_len, lengths_buf, 0);
}
		...
		int cvg_len)
{
	...
	if (get_ovflow_flag())
		warning("NAs produced by integer overflow");
	return integer_Rle_constructor(cvg_buf, cvg_len, NULL, 0);
}
		...
		int cvg_len)
{
	...
	return numeric_Rle_constructor(cvg_buf, cvg_len, NULL, 0);
}
		...
		SEXP method, RangeAE *ranges_buf)
{
	...
	if (out_ranges_are_tiles) {
		...
		if (take_short_path) {
			...
			return IS_INTEGER(weight) ?
			       integer_Rle_constructor(INTEGER(weight), x_len,
						       x_width, 0) :
			       numeric_Rle_constructor(REAL(weight), x_len,
						       x_width, 0);
		}
	}
