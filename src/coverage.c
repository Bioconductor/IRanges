...
SEXP IRanges_coverage(SEXP x, SEXP weight, SEXP width, SEXP method)
{
	...
	if (strcmp(method_ptr, "hash") == 0) {
		if (width_int == 0) {
			...
			*lengths_buf = width_int;
			...
			int *coverage = (int *) R_alloc((long) width_int, sizeof(int));
			...
			for (int i = 0; i < x_length; i++) {
				...
				for (int j = 0; j < x_width[i]; j++) {
					coverage[x_start[i]-1+j] += weight_val;
				}
			}
	...
	//original method. Memory efficient, but sorts incoming array
	else {
