...
%\VignetteDepends{}
%\VignetteKeywords{IntegerRanges}
...
concatenated with one or more atomic sequences to form larger
...
\subsubsection{Concatenating Vectors}

The \Rpackage{S4Vectors} and \IRanges{} packages use two generic functions,
\Rfunction{c} and \Rfunction{append}, for concatenating two \Rclass{Vector}
objects. The methods for \Rclass{Vector} objects follow the definition that
these two functions are given the \Rpackage{base} package.
...
like ``simple list" or ``compressed list", the \Rfunction{elementNROWS}
...
observation. The virtual \Rclass{IntegerRanges} class represents lists of
ranges, or, equivalently and as a derivative \Rclass{IntegerList},
sequences of consecutive integers. The most commonly used
implementation of \Rclass{IntegerRanges} is \Rclass{IRanges}, which stores
the starts and widths as ordinary integer vectors. To construct an
\Rclass{IRanges} instance, we call the \Rfunction{IRanges}
constructor. \Rclass{IntegerRanges} are normally specified by passing two
out of the three parameters: start, end and width (see \Rcode{help(IRanges)}
...
\Rclass{IntegerRanges} implementation.
<<iranges-start>>=
start(ir)
@
<<iranges-end>>=
...
For \Rclass{IRanges} and some other \Rclass{IntegerRanges} derivatives,
...
\Rclass{IntegerRanges} is, in fact, derived from \Rclass{IntegerList}.
<<ranges-extraction>>=
ir[[1]]
@
...
\Rclass{IntegerRanges} object as a set, where no elements are repeated and
order does not matter. While every \Rclass{IntegerRanges} object, as a
...
an \Rclass{IntegerRanges} object is said to be \textit{normal} when
its ranges are: (a) not empty (i.e. they have a non-null width);
...
\Rclass{IntegerRanges} object: (1) it guarantees a subsequence
...
The \Rfunction{reduce} function reduces any \Rclass{IntegerRanges}
...
\subsection{Lists of \Rclass{IntegerRanges} objects}

It is common to manipulate collections of \Rclass{IntegerRanges} objects
during an analysis. Thus, the \IRanges{} package defines some specific
classes for working with multiple \Rclass{IntegerRanges} objects.

The \Rclass{IntegerRangesList} class asserts that each element is an
\Rclass{IntegerRanges} object and provides convenience methods, such
as \Rfunction{start}, \Rfunction{end} and \Rfunction{width} accessors
that return \Rclass{IntegerList} objects, aligning with the
\Rclass{IntegerRangesList} object.
To explicitly construct an \Rclass{IntegerRangesList}, use the
...
As the elements of an \Rclass{IntegerRanges} object encode consecutive
subsequences, they may be used directly in sequence extraction. Note
that when a \textit{normal} \Rclass{IntegerRanges} is given as the index,
...
Utilities are available for transforming an \Rclass{IntegerRanges} object
...
are also returned in separate \Rclass{IntegerRanges} objects.
% 
<<ranges-threebands>>=
...
(integer) vector to an \Rclass{IntegerRanges} causes each range to be
...
@
%
The \Rfunction{*} operator symmetrically magnifies an \Rclass{IntegerRanges}
...
The \Rfunction{disjoin} function makes an \Rclass{IntegerRanges} object
...
Sometimes, it is useful to consider an \Rclass{IntegerRanges} object as a
...
\textit{difference} for \Rclass{IntegerRanges} objects. There are two
variants for each operation. The first treats each \Rclass{IntegerRanges}
...
\Rclass{IntegerRanges} objects and finds the range, if any, between
