...
example, into a list. However, as we strive to operate within the
formal S4 class system, we define a special collection class,
\Rclass{RleList}, for storing \Rclass{Rle} objects.
...
those functions, as well as a number of additional features.
