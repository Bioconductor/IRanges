...
% FIXME (wh 26.5.09): the above paragraph seems self-contradictory as 
% to what is actually done:
% - 'Behind the scenes, TypedList is simply an R list'. 
% - 'Our solution is to compress the list by concatenating the elements 
%    together'
...
% FIXME (wh 26.5.09): the term 'segments' would merit either
% a more precise definition, or perhaps better even introduction by
% way of example(s).
% Not clear what is meant by 'consecutive subsequences', 
% 'consecutive list of indices'. 
