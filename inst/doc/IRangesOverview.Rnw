...
compensate for this, \IRanges{} uses three generics, \Rfunction{endomorph},
...
The \Rfunction{endomorph} function performs an endomorphism equivalent to
...
the looping function \Rfunction{endomorph} to perform endomorphisms.

<<list-endomorph>>=
safe.max <- function(x) { if(length(x)) max(x) else integer(0) }
endomorph(sIntList2, safe.max)
endomorph(cIntList2, safe.max)
endomorph(sIntList2, safe.max)[[1]]
@
...
\Rfunction{viewMaxs}, \Rfunction{viewMins}, \Rfunction{viewSums}
