...
All \IRanges{}-derived sequences inherit from the \Rclass{Sequence}
virtual class.
...
\subsubsection{Sequence Subsetting}
...
these issues. \Rclass{List} is a direct extension of \Rclass{Sequence}.
...
\Rclass{Sequence}-derived object and, for a \Rclass{List}-derived object
